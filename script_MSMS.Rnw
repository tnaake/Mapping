...
##xset <- xcmsSet(method="centWave", ppm=30, snthresh=10, peakwidth=c(5,20), BPPARAM = MulticoreParam(workers = 6))
...
indNewFeat <- match(unique(as.character(MSMS_mod[, "id"])), as.character(MSMS_mod[,"id"]))
...
for (i in unique(pcgroups)) {
    ...
    if (length(indI) > 1) {
        ...
        precMZ <- lapply(strsplit(as.character(MSMS_mod[indI, "id"]), "_"), "[", 1)
...
truncatePeaklist <- function(peaklist, MSMS_mod, mode = c("WOS", "MJ")) {
    ...
    uniquePrecursor <- as.character(unique(MSMS_mod[, "id"]))
    for (i in 1:length(uniquePrecursor)) {
        inds <- which(MSMS_mod[, "id"] == as.character(uniquePrecursor[i]))
